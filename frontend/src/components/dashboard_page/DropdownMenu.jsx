...
function DropdownMenu({noteId}) {
  ...
  return (
    <Menu as="div" className="relative inline-block text-left">
      <Menu.Button><PiDotsThreeCircleLight className="w-8 h-8 text-white/60" /></Menu.Button>
      <Transition
        as={Fragment}
          ...
          <div className="p-4 grid">
            {icons.map((icon, index) => (
              <Menu.Item
                key={index}
              >
                <button onClick={() => handleClick(index)} className={`ui-active:bg-violet-500 ${icons.length - 1 === index && "ui-active:bg-red-700 mt-4 border-t-2"} ui-active:text-white font-bold ui-not-active:text-white group flex w-full items-center rounded-md px-2 py-2 text-sm`}>
                  {icon?.icon}
                  {icon?.label}
                </button>
              </Menu.Item>
